...
public class ColumnController {
    ...
    /**
     * Load columns for a specific table using the optimized ColumnDiscoveryService.
     *
     * @param props            Properties for application settings
     * @param tid              Table id
     * @param schema           Schema name
    ...
    public static void loadColumns(Properties props, Integer tid, String schema, String tableName, 
        ...
        try {
            // Validate inputs
            validateLoadColumnsInputs(props, tid, schema, tableName, connRepo, connDest, destRole);
    ...
    /**
     * Validate inputs for loadColumns method.
     * 
     * @param props Application properties
     * @param tid Table ID
     * @param schema Schema name
     * @param tableName Table name
     * @param connRepo Repository connection
     * @param connDest Destination connection
     * @param destRole Destination role
     */
    private static void validateLoadColumnsInputs(Properties props, Integer tid, String schema, 
                                                 String tableName, Connection connRepo, 
                                                 Connection connDest, String destRole) {
        if (props == null) {
            throw new IllegalArgumentException("Properties cannot be null");
        }
        if (tid == null || tid <= 0) {
        ...
        if (connDest == null) {
            throw new IllegalArgumentException("Destination connection cannot be null");
        }
        if (destRole == null || destRole.trim().isEmpty()) {
            throw new IllegalArgumentException("Destination role cannot be null or empty");
        }
    }
